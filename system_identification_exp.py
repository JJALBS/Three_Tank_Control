...
def activate_sensor(YOLO_path = r"parameters\YOLO.pt", 
                    ...
                    zero_time_duration = 2,
                    first_pump_speed = 14,
                    first_speed_duration = 15.0,
                    second_pump_speed = 10,
                    second_speed_duration = 15.0,
                    upper_valve = 5,
                    lower_valve = 6):
    ...
    while is_continue:
        ...
        if current_time - last_inference_time >= inference_interval:
            ...
            if current_time - start_time >= zero_time_duration*60:
                if current_time - start_time <= (zero_time_duration + first_speed_duration)*60:
                    pa.run_pump_speed(first_pump_speed)
                    mv_list.append(first_pump_speed)
                elif current_time - start_time <= (zero_time_duration + first_speed_duration + second_speed_duration)*60:
                    pa.run_pump_speed(second_pump_speed)
                    mv_list.append(second_pump_speed)
                else:
                    pa.run_pump_speed(0)
                    break
            else:
    ...
    return cv_list, time_list, mv_list
...
if __name__ == "__main__":
    cv_list, time_list, mv_list = activate_sensor(YOLO_path = r"parameters\YOLO.pt", 
                                        cam_index = 1, # often 0 is the installed webcam in laptop
                                        conf = 0.25, # recommend 0.9 or higher
                                        inference_interval = 1.0, # in seconds
                                        window_name = "Water Level Sensor",
                                        ...
                                        resolution_height = 480,
                                        resolution_width = 640,
                                        zero_time_duration = 1,
                                        first_pump_speed = 18,
                                        first_speed_duration = 15,
                                        second_pump_speed = 9,
                                        second_speed_duration = 15,
                                        upper_valve = 5,
                                        lower_valve = 6)
    ...
    base = rf"data\steady_state_map\trial.pickle"
