...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-sp", "--set_point", type=float, default=6, help="set point in cm."
    )
    parser.add_argument(
        "-c",
        "--controller",
    ...
    try:
        for cv in sensor:
            pa.run_digital_write(7, bool(np.random.choice([0,1]))) # #This is true/flase logic to operates the disturbance pump. The pump speed cannot be controlled. This is randomly run or stop the disturbance pump.
    ...
    os.makedirs(r"data\control_result\disturbance_lower_tank", exist_ok=True)
    for i in range(0, 1000):
        filename = rf"data\control_result\disturbance_lower_tank\{config["controller_type"]}_{i}.pickle"
